...
use super::super::{
    component::{
        AddressDetails, CancelInfoConfirmMsg, Frame, FrameMsg, PromptScreen, VerticalMenu,
...
impl FlowState for GetAddress {
    ...
    fn handle_event(&self, msg: FlowMsg) -> Decision<Self> {
        match (self, msg) {
            ...
            (GetAddress::Tap, FlowMsg::Confirmed) => Decision::Return(FlowMsg::Confirmed),
...
impl GetAddress {
    fn new(_args: &[Obj], kwargs: &Map) -> Result<Obj, error::Error> {
        ...
        // CancelTap
        let content_cancel_tap = Frame::left_aligned(title, PromptScreen::new_tap_to_cancel())
            .with_footer(TR::address__cancel_receive.into(), None)
            .map(|msg| match msg {
                FrameMsg::Content(()) => Some(FlowMsg::Confirmed),
                FrameMsg::Button(CancelInfoConfirmMsg::Cancelled) => Some(FlowMsg::Cancelled),
                _ => None,
