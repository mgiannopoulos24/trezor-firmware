...
secbool check_image_contents(const image_header *const hdr, uint32_t firstskip,
  ...
  uint32_t size = IMAGE_CHUNK_SIZE;
  const void *data = flash_area_get_address(area, firstskip, size - firstskip);
  if (!data) {
    return secfalse;
  }
  int remaining = hdr->codelen;
  if (sectrue !=
      check_single_hash(hdr->hashes, data, MIN(remaining, size - firstskip))) {
    return secfalse;
  }
